...
include_once("../timezones.php");
	...
	and (securelogin()==true) ) {
		if (isset($_POST['profileposts'])){
			...
			$result = mysqli_query($con,"SELECT * FROM posts WHERE userid='".$profileposts."' ");
			while($row = mysqli_fetch_array($result)){
				if ( (($row['userid']==$_COOKIE['id']) or ($row['super']=="true") or (in_array($row['userid'],$friends)) ) 
