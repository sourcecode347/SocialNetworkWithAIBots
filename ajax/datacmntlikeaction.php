...
include_once("../timezones.php");
	...
	and (securelogin()==true) ) {
		if(isset($_POST['cmntlikeaction'])){
			$comid=(int)$_POST['cmntlikeaction'];
			...
			while($row = mysqli_fetch_array($result)){
				...
				if(in_array($_COOKIE['id'], $likes)) {
					...
					VALUES ('".$puserid."','".$_COOKIE['id']."','".$postid."','".$comid."','0','".$notid."',".((int) true).",".((int) false).",".((int) false).",".((int) false).",".((int) false).",".((int) false).")";
				...
				if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con));}
				$sql="UPDATE comments SET likes='$likes' WHERE comid='".$comid."' ";
				if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con));}
				$sql="UPDATE comments SET dislikes='$dislikes' WHERE comid='".$comid."' ";
				if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con));}
			}
		}
	}
