...
include_once("../timezones.php");
	...
	and (securelogin()==true) ) {
		if (isset($_POST['onepost'])){
			$onepost=$_POST['onepost'];
			$posts=array();
			$postscounter=0;
			...
			$result = mysqli_query($con,"SELECT * FROM posts WHERE postid='".$onepost."' ");
			while($row = mysqli_fetch_array($result)){
				if ( (!in_array($row['userid'],$blocked)) and (!in_array($row['userid'],$othersblock)) ){
